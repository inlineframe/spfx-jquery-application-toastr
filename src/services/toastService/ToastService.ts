...
export class ToastService {
  ...
  public static getToasts(
    spHttpClient: SPHttpClient,
    baseUrl: string,
    webId: Guid
  ): Promise<IToast[]> {
    return new Promise<IToast[]>(
      (
        resolve: (toasts: IToast[]) => void,
  ...
  private static retrieveCache(webId: Guid): IToastCache {
    ...
    let cachedData: IToastCache = localStorage
      ? JSON.parse(localStorage.getItem(this.webStorageKey(webId)))
      : undefined;
    if (cachedData) {
      ...
      (
        ...
        if (cachedData.Loaded) {
          ...
          if (
            ...
            !ToastService.getFromListAlways
          ) {
  ...
  private static readonly apiEndPoint: string =
    "_api/web/lists/getbytitle('Toast')/items";
  private static readonly select: string =
    "Id,Title,Severity,Frequency,Enabled,Message";
  private static readonly orderby: string = "StartDate asc";

  /** Pulls the active toast entries directly from the underlying list */
  private static getToastsFromList(
    spHttpClient: SPHttpClient,
    baseUrl: string
  ): Promise<IToast[]> {
    ...
    return spHttpClient
      .get(
        `${baseUrl}/${ToastService.apiEndPoint}?$select=${ToastService.select}&$filter=${filter}&$orderby=${ToastService.orderby}`,
  ...
  private static indexOfToastStatusById(
    Id: number,
    toastStatuses: IToastStatus[]
  ): number {
  ...
  private static processCache(cachedData: IToastCache): IToastCache {
    ...
    cachedData.ToastStatuses = cachedData.ToastStatuses.filter(
      (value: IToastStatus): boolean => {
